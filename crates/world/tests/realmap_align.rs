...
use formats::otbm;
use protocol::map_description::{Center, OPCODE_MAP_DESCRIPTION, encode};
...
fn real_spawn_viewport_aligns_with_client_parser() {
    let items_bytes = match std::fs::read("../../reference/tfs/data/items/items.otb") {
        Ok(b) => b,
        Err(_) => {
            eprintln!("skip: no reference items.otb");
            return;
        }
    };
    let map_bytes = match std::fs::read("../../reference/tfs/data/world/forgotten.otbm") {
        Ok(b) => b,
        Err(_) => {
            eprintln!("skip: no reference map");
            return;
        }
    };
    ...
    for it in &items.items {
        by_cid.entry(it.client_id).or_insert(it);
    }
            ...
            if peek >= 0xFF00 {
                ...
                loop {
                    ...
                    if let Some(it) = by_cid.get(&v) {
                        if it.flags & FLAG_STACKABLE != 0 || it.group == 11 || it.group == 12 {
                            p += 1;
                        }
                        if it.flags & FLAG_ANIMATION != 0 {
                            p += 1;
                        }
                    }
                }
    ...
    assert!(
        !over,
        "client parser over-read (EOF) — wire still misaligned at p={p}/{}",
        bytes.len()
    );
    let leftover = bytes.len() as i64 - p as i64;
    assert_eq!(
        leftover, 0,
        "leftover bytes after client-faithful parse: {leftover}"
    );
}
