...
impl Game {
    ...
    pub(super) fn take_from_ground(&mut self, from: Position, src_idx: usize, want: u8, stackable: bool) -> Option<(u8, bool)> {
    ...
    pub(super) fn do_move_thing(&mut self, id: u32, from: Position, from_stackpos: u8, to: Position, count: u8) {
    ...
    pub(super) fn do_move_inventory(&mut self, id: u32, from: Position, from_stackpos: u8, to: Position, count: u8) {
    ...
    pub(super) fn broadcast_dest(&mut self, pos: Position, sp: u8, item: WireItem, is_update: bool) {
    ...
    pub(super) fn broadcast_source(&mut self, pos: Position, from_stackpos: u8, removed_fully: bool, src_idx: usize) {
        let mut targets = self.spectators(pos, u32::MAX);
        targets.extend(
