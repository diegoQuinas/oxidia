...
The system MUST compute its own A* path via `get_path_matching()` from the player's current position. Client-sent direction steps MUST NOT be followed — only the destination is derived from them.
